      ...
      <div className="relative">
                  ...
                  <div className={`p-4 rounded-lg border border-gray-200 dark:border-gray-700 
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className={`inline-flex items-center gap-1 text-xs 
                                        ${item.impact === 'disruptive' ? 'text-red-600 dark:text-red-400' : 
                                          item.impact === 'emerging' ? 'text-green-600 dark:text-green-400' : 
                                          item.impact === 'incremental' ? 'text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400'}`}>
                          {item.impact === 'disruptive' ? '🔥 Disruptif' : 
                           item.impact === 'emerging' ? '🌱 Émergent' : 
                           item.impact === 'incremental' ? '📈 Incrémental' : '📉 Déclinant'}
                        </span>
                      </div>
        ...
        {visibleItems < filteredItems.length && (
          <div className="flex justify-center mt-6">
            <button
              onClick={() => setVisibleItems(prev => Math.min(prev + 5, filteredItems.length))}
        ...
        {filteredItems.length === 0 && (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            ...
            <p>Aucun insight trouvé avec ce filtre</p>
          </div>
        )}
      </div>
    </div>

      ...
      <div className="flex items-end h-8 gap-1">
        {trend.map((value, index) => {
          ...
          return (
            <div
              key={index}
              className={`w-1 ${getColorClasses(color as KPIData['color']).accent} opacity-60 rounded-full transition-all duration-300`}
        ...
        return (
          ...
          >
            ...
            >
              {/* Face avant */}
              <div
                className={`absolute inset-0 w-full h-full backface-hidden rounded-xl p-6 
                           border-2 ${colors.bg} ${colors.border} hover:shadow-lg transition-all duration-300
                  ...
                  <div>
                    ...
                    <div className={`text-3xl font-bold ${colors.value} mb-2`}>
                      {typeof kpi.value === 'number' ? (
                        <CountUpAnimation target={animatedValue} />
                      ) : (
                        kpi.value
                      )}
                    </div>
              ...
              {/* Face arrière */}
              <div
                className={`absolute inset-0 w-full h-full backface-hidden rotate-y-180 rounded-xl p-6 
                           border-2 ${colors.bg} ${colors.border} hover:shadow-lg transition-all duration-300`}
              >
